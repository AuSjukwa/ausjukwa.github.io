...
import workText from 'raw-loader!./views/work.txt';
import ternimalText from 'raw-loader!./views/terminal.txt';
import headerHTML from 'raw-loader!./views/header.html';
let styleText = [0, 1, 2, 3].map((i) => require('raw-loader!./styles/styles' + i + '.css').default);
import preStyles from 'raw-loader!./styles/prestyles.css';
import replaceURLs from './lib/replaceURLs';
import {default as writeChar, writeSimpleChar, writeTerminalChar, handleChar, handleTerminalChar} from './lib/writeChar';
import getPrefix from './lib/getPrefix';

// Vars that will help us get er done
const isDev = window.location.hostname === 'localhost';
const speed = isDev ? 0 : 16;
let style, styleEl, workEl, ternimalEl, skipAnimationEl, pauseEl;
...
async function startAnimation() {
  try {
    ...
    await writeTo(ternimalEl, ternimalText, 0, speed, 'terminal', 1);
...
async function surprisinglyShortAttentionSpan() {
  ...
  // terminal text
	let terminalHTML = '';
	for (let i = 0; i < ternimalText.length; i++) {
		terminalHTML = handleTerminalChar(terminalHTML, ternimalText[i]);
	}
  ternimalEl.innerHTML = terminalHTML;

  // There's a bit of a scroll problem with this thing
  let start = Date.now();
  while(Date.now() - 1000 > start) {
    workEl.scrollTop = Infinity;
    styleEl.scrollTop = ternimalEl.scrollTop = Infinity;
...
function getEls() {
  ...
  ternimalEl = document.getElementById('ternimal-text');
...
function createWorkBox() {
  ...
  // flippy floppy
  let flipping = 0;
  require('mouse-wheel')(workEl, async function(_dx, dy) {
    if (flipping) return;
